...
// const data = require('./wsp-contract-data.json');
...
async function generateWSPContractPdf(data) {
    try {
        let pdf_directory = path.join(__dirname, 'wsp-contracts');
        ...
        const content = await compile('wspContractTemplate', data);  // Pass the transformed purchase data
        
        // Compile to HTML file
        const htmlFilePath = path.join(__dirname, 'compiled-wspContractTemplate.html');
        await fs.writeFile(htmlFilePath, content);

        await page.goto(`file:${htmlFilePath}`, { waitUntil: 'networkidle0' });

        const pdfPath = path.join(__dirname, 'wsp-contracts', file_name);
        // const pdfPath = path.join(__dirname, 'wsp-contracts', 'test-wsp-contract.pdf');
        await page.pdf({
            path: pdfPath,
            format: 'A4',
...
(async () => {
    try {
        console.log(generateWSPContractPdf());
    } catch (error) {
        console.error('Error: ', error);
    }
})

module.exports.generateWSPContractPdf = generateWSPContractPdf;